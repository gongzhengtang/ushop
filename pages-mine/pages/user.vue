<template>
	<view class="page">
		...
		<!-- 信息 -->
		<view class="info">
			<view class="avatar"><u-avatar size="132" src="http://120.76.202.58/static/user/1.png"></u-avatar></view>
		...
		<view class="item" v-if="currentTab == 0">
			...
			<view class="card" style="padding-bottom: 0;">
				...
				<CommunityCard showActive v-for="(item, index) in communityList"
					:border="communityList.length - 1 != index" :data="item" :key="index"></CommunityCard>
			</view>
		</view>
		<view class="item" v-if="currentTab == 1">
			<!--帖子卡片-->
			<view class="card" style="padding: 0;">
				<PostCard smallCard v-for="(item, index) in circleList" :key="index" :data="item"
