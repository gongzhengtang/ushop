<template>
	<view class="slot" :style="showBorderBottom ? '' : 'border-bottom:none;margin-bottom:0'">
		<view class="user">
			<view class="info">
				<u-avatar size="80" src="http://120.76.202.58/static/user/1.png"></u-avatar>
				<view class="basic">
					<view class="name">{{ data.userName }}</view>
					<view class="evaluate"><u-rate :count="5" v-model="data.star" disabled
							:inactive-color="rateInActiveColor" :active-color="rateActiveColor"></u-rate></view>
				</view>
			</view>
			<view class="date">{{ data.date }}</view>
		</view>
		<view class="content">{{ data.content }}</view>
		<view class="pictures"><u-image width="164rpx" height="164rpx" src="../../../static/goods/4.png"
				v-for="(pic, cur) in data.pics" :key="cur"></u-image></view>
	</view>
</template>
