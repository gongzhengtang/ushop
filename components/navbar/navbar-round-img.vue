<template>
	<view class="slot">
		<u-navbar :back-icon-name="backIconName" :border-bottom="false" :is-back="isBack"
			...
			:back-icon-color="backIconColor || appThemeTextWhiteColor"
			:background="{ backgroundColor: backgroundColor || appThemeColor }" :title="title"></u-navbar>
		<view class="bg-img" :style="{ height }"><u-image width="100%" :src="bgImg" mode="widthFix"></u-image></view>
	</view>
</template>
...
<script>
	export default {
		...
		props: {
			...
			bgImg: {
				type: String,
				default: 'http://120.76.202.58/static/bg/bg.png'
			},
			// 是否带返回按钮
			isBack: {
				type: Boolean,
