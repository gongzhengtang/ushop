	...
	<view class="slot">
		<view class="slot-inner" :style="[
			...
			<view class="search-slot" :style="[{ minHeight: menuHeight + 'px' }]">
				<view @click="$u.route({ type: 'navigateBack', delta: 1 })" class="left"><u-icon size="32"
						name="arrow-leftward"></u-icon></view>
				<u-search :placeholder="placeholder" :showAction="false" shape="square" borderRadius="0rpx"
					:bg-color="searchBgColor" :style="{ width: 'calc(100% - 100rpx - ' + menuWidth + 'px)' }"
					:class="showSlot ? 'active' : ''" :value="value" @change="searchChange"
					@confirm="searchConfirm"></u-search>
			</view>
			<view class="tabList" :style="{ paddingTop: '16px' }" v-if="showTabs">
