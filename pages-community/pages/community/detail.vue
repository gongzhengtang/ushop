<template>
	<view class="page">
		<!-- 一般导航栏 -->
		<NavbarRoundImg :titleColor="titleColor" :backIconColor="backIconColor" height="420rpx" isBack title="圈子主页"
			:bgImg="bgImg" :backgroundColor="backgroundColor"></NavbarRoundImg>
		<view class="shadow">
			<!-- 圈子头部 -->
		...
		<view class="post-list">
			<Tabs :backgroundColor="bgColor" padding="0" :listData="scrollData"
				:activeBarColor="appThemeTextBlackColor"></Tabs>
			<!--帖子卡片-->
			<PostCard v-for="(item, index) in circleList" :key="index" :data="item"
				:border="index != circleList.length - 1"></PostCard>
		</view>
		<!-- 加入圈子弹窗 -->
		<u-modal confirm-text="加入圈子" :confirm-color="appThemeColor" show-cancel-button v-model="showAddCommunity"
			:content="'是否加入“' + communityTitle + '”？'"></u-modal>
		<!-- 添加按钮 -->
	...
	export default {
		...
		data() {
			return {
				...
				addOps: [{
						label: '发布帖子',
						icon: 'http://120.76.202.58/static/operate/publish-post.png',
						url: '/pages-community/pages/post/add',
						type: 'page'
					},
					{
						label: '编辑圈子',
						icon: 'http://120.76.202.58/static/operate/edit-community.png',
						url: '/pages-community/pages/community/add-or-update?type=update',
						type: 'page'
					},
					{
						label: '解散圈子',
						icon: 'http://120.76.202.58/static/operate/remove-community.png',
						url: '',
						type: 'remove'
					}
				],
				...
				shareOps: [{
						label: '发给微信好友',
						icon: 'http://120.76.202.58/static/operate/wechat-share.png',
						url: '',
						type: 'share'
					},
					{
						label: '复制链接',
						icon: 'http://120.76.202.58/static/operate/copy-url.png',
						url: '',
						type: 'copy'
					}
				],
